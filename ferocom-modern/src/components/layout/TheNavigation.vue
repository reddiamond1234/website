<template>
  <nav class="fixed top-[90px] left-0 w-full bg-primary-600 flex items-center z-40 shadow-md">
    <!-- Mobile hamburger menu -->
    <button
      class="md:hidden text-2xl p-3 text-white w-full text-center hover:bg-primary-700 transition-colors"
      @click="toggleMenu"
    ...
    <ul
      :class="[
        'w-full m-0 p-0 list-none md:flex md:justify-around',
    ...
    >
      <li
        ...
        :key="item.path"
        :class="[
          'flex-1 text-center py-4 cursor-pointer text-white border-r border-primary-700 last:border-r-0 transition-colors',
      ...
      <li class="relative flex-1 text-center text-white border-r-0 hover:bg-primary-700 transition-colors">
        <button
          ...
          class="flex items-center justify-center gap-2 w-full h-full py-4 px-4"
        >
